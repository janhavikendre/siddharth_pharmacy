...
export default function HeroBanner() {
  ...
  return (
    <section className="w-full h-[500px] md:h-[600px] flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-1/2 relative flex items-center justify-center">
        ...
        <div className="relative z-20 flex flex-col items-center justify-center text-center w-full h-full px-4 sm:px-8 py-8 md:py-12 text-white">
          ...
          <h1 className="text-[2rem] sm:text-[2.5rem] md:text-[3.5rem] lg:text-[4rem] xl:text-[4.5rem] font-extrabold mb-6 md:mb-10 leading-[1.1] md:leading-[1.08] tracking-tight">
            We Provide High<br />
            Quality <span className="underline decoration-green-400 decoration-[6px] underline-offset-[10px]">Education</span><br />
            For Everyone
          </h1>
      ...
      <div className="w-full md:w-1/2 h-[250px] md:h-full relative overflow-hidden">
        <div className="w-full h-full relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white p-2 sm:p-4">
            <h1 className="text-lg sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-4 max-w-2xl">{slides[currentSlide].title}</h1>
            <p className="text-sm sm:text-base md:text-lg mb-4 sm:mb-8 max-w-xl">{slides[currentSlide].description}</p>
            <Button asChild size="lg" className="bg-blue-600 hover:bg-blue-700 text-xs sm:text-base">
          ...
          <div className="absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 flex space-x-1 sm:space-x-2">
            {slides.map((_, index) => (
              <button
                ...
                className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${index === currentSlide ? "bg-white" : "bg-white/50"}`}
